...
import { SUB_STATUSES } from "@/types/candidate";

const STEPS = [
  { key: "applied", label: "応募", icon: "📝" },
  { key: "setup", label: "設置", icon: "📅" },
  { key: "conducted", label: "実施", icon: "💬" },
  { key: "supporting", label: "サポート中", icon: "📋" },
  { key: "offered", label: "内定", icon: "🎉" },
  { key: "accepted", label: "内定承諾", icon: "🤝" },
...
export default function ProgressTimeline({ candidateStatus }: Props) {
  ...
  const isFailed = SUB_STATUSES.includes(candidateStatus as (typeof SUB_STATUSES)[number]);
      ...
      <div className="hidden sm:flex items-start justify-between relative">
        ...
        <div
          ...
          style={{
            width: isFailed
              ? "0%"
              : `${Math.max(0, (currentIndex / (STEPS.length - 1)) * 100 - 5)}%`,
              ...
              <div className="flex flex-col items-center">
                ...
                {!isLast && (
                  <div
                    ...
                    style={{
                      backgroundColor:
                        isDone && currentIndex > i ? "#0649C4" : "#E5E7EB",
                    }}
                  />
                )}
              </div>
