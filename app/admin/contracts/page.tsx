...
export default function ContractsPage() {
  ...
  const [form, setForm] = useState({
    company_id: "", title: "", content: "", status: "draft" as ContractStatus,
  ...
  function openCreate() {
    setForm({ company_id: "", title: "", content: "", status: "draft", start_date: "", end_date: "", file_url: "" });
    setEditingId(null);
    setModalOpen(true);
    setError("");
  }
  ...
  function openEdit(c: Contract) {
    setForm({
      company_id: c.company_id ?? "", title: c.title, content: c.content ?? "",
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#002D37]">契約書管理</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="px-6 py-12 text-center text-[#6B7280]">契約書が登録されていません</div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="text-left px-6 py-3 text-xs font-semibold text-[#6B7280] uppercase tracking-wider">開始日</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-[#6B7280] uppercase tracking-wider">終了日</th>
            ...
            <tbody>
              {contracts.map((c) => (
                <tr key={c.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                  <td className="px-6 py-4 text-sm text-[#002D37]">{c.company_name ?? "-"}</td>
                  <td className="px-6 py-4 text-sm font-medium text-[#002D37]">
                    <Link href={`/admin/contracts/${c.id}`} className="hover:underline">{c.title}</Link>
                  </td>
                  <td className="px-6 py-4"><StatusBadge status={c.status} /></td>
                  <td className="px-6 py-4 text-sm text-[#6B7280]">{formatDate(c.start_date)}</td>
                  <td className="px-6 py-4 text-sm text-[#6B7280]">{formatDate(c.end_date)}</td>
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => openEdit(c)} className="text-sm text-[#002D37] hover:underline cursor-pointer mr-4">編集</button>
                    <button onClick={() => setDeleteTarget(c)} className="text-sm text-red-600 hover:underline cursor-pointer">削除</button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[#002D37] mb-1">開始日</label>
                  <input type="date" value={form.start_date} onChange={(e) => setForm({ ...form, start_date: e.target.value })}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#002D37]/20 focus:border-[#002D37]" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-[#002D37] mb-1">終了日</label>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            <h3 className="text-lg font-semibold text-[#002D37] mb-2">契約書の削除</h3>
